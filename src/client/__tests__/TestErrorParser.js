import {isString, capitalise, camelCaseToVerbose, underscoredToVerbose, parseErrors} from '../ErrorParser'
...
describe('API and helpers', () => {
  ...
  it('can convert camel case', () => {
    expect(camelCaseToVerbose('ThisIsInCamelCase')).toEqual('This Is In Camel Case')
    expect(camelCaseToVerbose('This_is_not')).toEqual('This_is_not')
    expect(camelCaseToVerbose('')).toEqual('')
  })

  it('can convert snake case', () => {
    expect(underscoredToVerbose('this_is_in_snake_case')).toEqual('this is in snake case')
