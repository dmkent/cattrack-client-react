...
describe('components', () => {
  describe('TransactionFilter', () => {
    ...
    it('should display some categories and accounts', () => {
      ...
      expect(enzymeWrapper.find('Button').at(2).text()).toBe("Cat1")
      expect(enzymeWrapper.find('Button').at(2).props().active).toBe(false)
      expect(enzymeWrapper.find('Button').at(3).text()).toBe("Cat2")
      expect(enzymeWrapper.find('Button').at(3).props().active).toBe(false)
      enzymeWrapper.find('Button').at(2).props().onClick()
    ...
    it('should display some periods and set active', () => {
      ...
      expect(enzymeWrapper.find('Button').at(3).text()).toBe("Cat2")
      expect(enzymeWrapper.find('Button').at(3).props().active).toBe(true)

      // reset to "All"
      enzymeWrapper.find('Button').at(1).props().onClick()
      expect(props.onFilter.mock.calls.length).toBe(1)
      expect(props.onFilter.mock.calls[0][0]).toEqual({category: null})

      // reset account to "All"
      enzymeWrapper.find('Button').at(4).props().onClick()
      expect(props.onFilter.mock.calls.length).toBe(2)
