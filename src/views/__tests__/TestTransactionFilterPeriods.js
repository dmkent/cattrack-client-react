...
describe('components', () => {
  describe('TransactionFilterPeriods', () => {
    it('should render self and subcomponents', () => {
      ...
      expect(props.loadPeriods.mock.calls.length).toBe(1)
    })

    it('changing dates should change filter', () => {
      const { enzymeWrapper, props } = setup([], {})

      enzymeWrapper.find('DateRangePicker').props().onDatesChange({
    ...
    it('should display some periods', () => {
      const { enzymeWrapper, props } = setup([
          {id: 0, offset: 0, from_date: "2017-01-01", to_date: "2017-02-01", label: "Month"},
    ...
    it('should detect null current filter', () => {
      ...
      expect(props.onFilter.mock.calls[1][0]).toEqual({from_date: null, to_date: null})
    })
  })
})