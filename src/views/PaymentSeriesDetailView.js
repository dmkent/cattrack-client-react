...
import {Form, FormControl, FormGroup, ControlLabel, Button, Col, Row} from 'react-bootstrap';
        ...
        return (
          <Row>
            <Col md={12}>
              ...
              <UploadForm submit_label="Add" uploadFile={(data) => this.props.paymentSeriesAddBillFromFile(this.props.series.id, data)}/>
              <p>
                {
              ...
              {(this.props.series.bills.size > 0) && 
               [...this.props.series.bills.values()].map(bill => {
                return (
                  <li key={bill.id}><span>{bill.due_date} </span>
                    <FormattedNumber value={bill.due_amount || 0.0}
                                     style="currency"
                                     currency="AUD"/>
                  </li>
                )
