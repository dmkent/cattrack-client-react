...
describe("Account actions", () => {
  ...
  it("should create file upload actions", () => {
    ...
    const store = mockStore();
    return store
      .dispatch(TrackActions.uploadToAccount({ id: 1 }, "dummy file object"))
      .then(() => {
        // Return of async actions
        expect(store.getActions()).toEqual(expectedActions);
      });
  ...
  it("should create new account actions", () => {
    ...
    return store.dispatch(TrackActions.createAccount("newacct")).then(() => {
      // Return of async actions
      expect(store.getActions()).toEqualImmutable(expectedActions);
    });
  });
  ...
  it("should create account balance series load actions", () => {
    ...
    return store
      .dispatch(
        TrackActions.loadAccountBalanceSeries(
          new Account({ id: 3, name: "newacct" })
        )
      )
      .then(() => {
        // Return of async actions
  ...
  it("should create account balance series load error actions", () => {
    ...
    return store
      .dispatch(
        TrackActions.loadAccountBalanceSeries({ id: 3, name: "newacct" })
      )
      .then(() => {
        // Return of async actions
        expect(store.getActions()).toEqual(expectedActions);
  ...
  it("should create account select actions", () => {
    expect(
      TrackActions.selectAccount({ id: 3, name: "test" })
    ).toEqualImmutable({
